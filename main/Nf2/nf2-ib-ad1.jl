...
""" 
function correlators(data, corrws, masses, u1ws, nsrc)
    reset_data(data)
    for isrc in ProgressBar(1:nsrc)
        for it in 1:N0
            random_source(it,corrws,masses,u1ws)
            for ifl in 1:2
...
function save_data(data, dirpath, cnfg)
    fname = joinpath(dirpath,"measurements/2pt-stoc-ib-conn-disc_n$cnfg.h5")
    fid = h5open(fname, "w")
...
for i in ProgressBar(start:finish)
    ...
    correlators(data, pws, [am01, am02], model_s, NSRC)
