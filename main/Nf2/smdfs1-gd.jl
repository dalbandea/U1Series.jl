# ===========================================================================
# U(1) Nf=2 SMD with a FormalSeries-valued mass AND gauge damping (NSPT).
#
# Same as smdfs1.jl, but the sampler is the gauge-damped U1Nf2SMDgd
# (src/U1SeriesGaugeDamping.jl): a Monte-Carlo-time-dependent gauge
# transformation removes the longitudinal zero mode that would otherwise make
# the propagated mass-derivatives random-walk. Specialisation to U(1) of
# Dalla Brida & Lüscher, arXiv:1703.04396 (sec. 5.4).
#
# The extra SMD param is `lambda0` (gauge-damping strength). Stability requires
# eps^2 * lambda0 * ||Delta|| < kappa (2D: ||Delta||=8). NB: under the GHMC
# structure the refresh fires once per trajectory with c1 = exp(-gamma*eps), so
# use a large enough `gamma` for the damping to keep up (e.g. gamma ~ nsteps).
# ===========================================================================
...
lambda0    = pdata["SMD params"]["lambda0"]     # gauge-damping strength
...
samplerws = U1Nf2SMDgd(model_s, smplr; lambda0 = lambda0)

logio = open(joinpath(wdir, savename * "_log.txt"), "a+")
logger = SimpleLogger(logio)
global_logger(logger)

@info "U(1) NF=2 SERIES GAUGE-DAMPED SMD SIMULATION (NDER = $NDER)" model_s.params smplr lambda0
